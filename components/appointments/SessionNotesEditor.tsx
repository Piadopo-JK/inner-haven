...
export default function SessionNotesEditor({
  ...
  const [followUp, setFollowUp] = useState(note?.follow_up ?? "");
  const [message, setMessage] = useState<string>("");
  const [error, setError] = useState<string>("");

  const hasExistingNote = useMemo(() => Boolean(note), [note]);
    ...
    return (
      <div className="space-y-6">
        <section className="rounded-2xl border bg-[var(--md-sys-color-surface-container-low)] p-5" style={sectionStyle}>
          ...
          <p className="text-sm whitespace-pre-wrap" style={bodyTextStyle}>
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border bg-[var(--md-sys-color-surface-container-low)] p-5" style={sectionStyle}>
        ...
        {editing ? (
          ...
          <p className="text-sm whitespace-pre-wrap" style={bodyTextStyle}>
            {note?.note_content || "No notes yet."}
          </p>
        )}
      </section>
      ...
      <section className="rounded-2xl border bg-[var(--md-sys-color-surface-container-low)] p-5" style={sectionStyle}>
        ...
        {editing ? (
          ...
          <p className="text-sm whitespace-pre-wrap" style={bodyTextStyle}>
            {note?.follow_up || "No follow-up plan yet."}
          </p>
        )}
      </section>
